...
создаёт временные копии карт без сохранённого состояния открытых UI (оно не
влияет на геометрию или спавнеры и может содержать невалидные ссылки), после
чего использует `Content.MapRenderer` и формирует ленивую тайловую пирамиду.
