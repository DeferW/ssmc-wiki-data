...
Изображение создаёт штатный `Content.MapRenderer` самой игры без уменьшения:
максимальный zoom сохраняет исходные 32 пикселя на игровую клетку в lossless
WebP. Сборщик режет изображение на разреженные тайлы 512×512 и строит уменьшенные
уровни для навигации. Сырые рендеры кэшируются по commit игры и версии
предрендерной очистки, поэтому повторная упаковка не запускает игру заново.
Небольшой патч рендера добавляет в `tiles.json` исходную нижнюю левую координату
обрезанной сетки (`worldMin`): без неё игровые координаты спавнеров невозможно
точно совместить с изображением. Он также исправляет ошибку штатного painter-а,
который применял горизонтальное смещение обрезки к вертикальной координате
стен и предметов, из-за чего слой сущностей расходился со слоем пола.
Исправленные изображения публикуются как третья версия тайлового контракта,
чтобы клиент не переиспользовал старые WebP из браузерного кэша.
Полноразмерный промежуточный WebP кодируется быстрым lossless-методом: его
пиксели остаются идентичными, а окончательное сжатие выполняется уже на тайлах.

Набор карт также определяется из игровых прототипов. Сборщик всегда добавляет
«Алмайер», а планеты берёт из `RMCPlanetMapPrototype` только при
`inRotation: true` (это штатное значение по умолчанию). Поэтому PVE-карты и
прочие отключённые прототипы не рендерятся, но новые карты основной ротации
подхватываются автоматически без ручного списка путей.

Клиент сначала загружает только `catalog.json`. При открытии карты он получает
её отдельные `tiles.json` и `overlay.json`, после чего запрашивает только тайлы
текущего масштаба, пересекающие видимую область. Остальные карты и невидимые
части открытой карты не должны загружаться. Защитный лимит всего `data/maps/` —
512 MiB; это внутренний бюджет сборщика, а не лимит одного запроса браузера.
