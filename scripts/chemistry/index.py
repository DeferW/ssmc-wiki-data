...
from scripts.common.yaml import GameYamlLoader, normalize_parents
...
def main() -> None:
    ...
    if duplicates:
        details = "; ".join(
            f"{key}: {', '.join(paths)}" for key, paths in sorted(duplicates.items())
        )
        raise RuntimeError("Duplicate chemistry prototype IDs: " + details)
